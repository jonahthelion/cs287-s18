...
class noAttention(nn.Module):
    ...
    def get_encode(self, src):
        ...
        output, hidden = self.encoder(embedded, (Variable(torch.zeros(self.num_encode, src.shape[0], self.D).cuda()), Variable(torch.zeros(self.num_encode, src.shape[0], self.D).cuda())))
