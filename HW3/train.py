...
with open(fname, 'rb') as reader:
    for break_ix, line in tqdm(enumerate(reader)):
        ...
        while len(actual_sentences) < 100:
            poss_hidden = torch.stack([output for _ in range(poss_sentences.shape[1])], 1), (torch.stack([hidden[0][:,0] for _ in range(poss_sentences.shape[1])], 1), torch.stack([hidden[1][:,0] for _ in range(poss_sentences.shape[1])], 1))
