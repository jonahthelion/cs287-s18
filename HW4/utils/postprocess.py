...
def vis_display(vis, vis_windows, x_coord, train_l, MAP, sample_img, val_kl, val_recon):
    if vis_windows is None:
        ...
        vis_windows['val_recon'] = vis.line(Y=torch.Tensor([float(val_recon)]), X=torch.Tensor([x_coord]), opts=dict(title='Validation XENT', ytickmax=200, ytickmin=0))
        vis_windows['val_kl'] = vis.line(Y=torch.Tensor([float(val_kl)]), X=torch.Tensor([x_coord]), opts=dict(title='Validation KL', ytickmax=10, ytickmin=0))
        ...
        if not val_kl is None:
            vis.line(Y=torch.Tensor([float(val_recon)]), X=torch.Tensor([x_coord]), win=vis_windows['val_recon'], update='append', opts=dict(title='Validation XENT', ytickmax=200, ytickmin=0))
            vis.line(Y=torch.Tensor([float(val_kl)]), X=torch.Tensor([x_coord]), win=vis_windows['val_kl'], update='append', opts=dict(title='Validation KL', ytickmax=10, ytickmin=0))
