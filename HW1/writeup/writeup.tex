...
All words are initialized to a count of $\alpha$. Figure \ref{fig:clusters} shows that performance of the algorithm on the validation set is robust to changes in $\alpha$. With $\alpha=.5$, performance on the test set using binarized and count word vectors are analyzed in Table \ref{tab:nb}.
...
\begin{tabular}{llrrr}
 ...
 Model &  & Acc. & Bce. & Roc.\\
 \midrule
 \textsc{Binarized} & & 0.79 & 0.68 & 0.87\\
 \textsc{Counts} & & 2.59  & 1 & 1\\
 \bottomrule
\end{tabular}
\caption{\label{tab:nb} Table with the main results.}
\end{table}
