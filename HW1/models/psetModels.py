...
class Conv(nn.Module):
    def __init__(self, V, embed):
        ...
        self.w3 = nn.Sequential(
            nn.Conv1d(in_channels=300, out_channels=50, kernel_size=3, stride=1, padding=0),
            nn.ReLU(inplace=True),
            nn.AdaptiveMaxPool1d(1))
        self.w4 = nn.Sequential(
            nn.Conv1d(in_channels=300, out_channels=25, kernel_size=4, stride=1, padding=0),
            nn.ReLU(inplace=True),
            nn.AdaptiveMaxPool1d(1))
        self.w5 = nn.Sequential(
            nn.Conv1d(in_channels=300, out_channels=25, kernel_size=5, stride=1, padding=0),
            nn.ReLU(inplace=True),
            nn.AdaptiveMaxPool1d(1))
